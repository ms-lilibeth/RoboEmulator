...
class Robot:
    ...
    _distance_per_tick = 0  # = max(l_power, r_power)*speed_coefficient
    # the width of each wheel is 2 (ref. specification.pdf)

    # For not to count them every tick
    _le_power_changed = True
    _re_power_changed = True

    _le_turning_radius = 0  # in case of left_engine_forward
    _le_turning_angle = 0  # in case of left_engine_forward
    _le_coords_delta = 0  # in case of left_engine_forward

    _re_turning_radius = 0  # in case of right_engine_forward
    _re_turning_angle = 0  # in case of right_engine_forward
    _re_coords_delta = 0  # in case of right_engine_forward

    _be_turning_radius = 0  # in case of both_engines_forward
    _be_turning_angle = 0  # in case of both_engines_forward
    _be_coords_delta = 0  # in case of both_engines_forward

    """Parameters description:
    depict_callback - a function for drawing the robot that takes three parameters: (dx, dy), angle, rotation_point
    (where x and y are the coordinates of the upper-left corner; rotation_point -- a tuple that contains coordinations
    of the point to rotate around)
    x, y -- coordinates of the upper-left corner
    angle -- an angle relative to y-axis (in degrees)
    """
    def __init__(self, depict_callback):
        self.depict_callback = depict_callback
    ...
    @staticmethod
    def _get_coords_delta(self, radius, angle):
